...
export const airtimeTopUp = catchAsync(async (req, res, next) => {
  const { customerId, billerCode, category } = req.body;
  ...
  if (!customerId || !billerCode || !category) {
    return next(
      new AppError(
  ...
  try {
    await axios.get('https://vtu.ng/wp-json/api/v1/airtime', {
      params: {
        username,
        password,
        phone: customerId,
